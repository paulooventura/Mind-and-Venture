...
const TMJ_PLAYER_SPAWN_GID = 150;
const TMJ_SPAWN_FIRST_GID = 150;
...
const TMJ_PLAYER_SPAWN_GIDS = new Set([150,8200,6683,6684]);
const TMJ_MIND_SPAWN_GIDS = new Set([204]);
const TMJ_RCA_SPAWN_GIDS = new Set([168]);
...
const TMJ_OMNIBLOCK_FIRST=145;
const TMJ_OMNIBLOCK_FRAMES=5;
const TMJ_SPAWN_TILE_COUNT=64;
function _isOmniblockGid(gid){ return gid>=TMJ_OMNIBLOCK_FIRST&&gid<TMJ_OMNIBLOCK_FIRST+TMJ_OMNIBLOCK_FRAMES; }
...
let _tmjDraw=null, _tmjImg=null, _tmjImgCols=12, _mapReady=false, _candleLights=[];
const TMJ_TILESET_DEFS=[
  {firstGid:1,lastGid:144,cols:12,img:null,paths:['assets/home baked sprites/material/MV2 tilesheet.png']},
  {firstGid:145,lastGid:149,cols:5,img:null,paths:['assets/home baked sprites/square/omniblock.png','assets/home baked sprites/square/omniblock.gif']},
  {firstGid:150,lastGid:149+TMJ_SPAWN_TILE_COUNT,cols:8,img:null,paths:['assets/home baked sprites/material/spawn spots.png']},
  {firstGid:8025,lastGid:8984,cols:40,img:null,paths:['assets/tilesetOpenGame.png','tileset/tilesetOpenGame.png','tileset/mindventure_tileset_8x8.png','tilesetOpenGame.png']},
  {firstGid:8985,lastGid:8988,cols:2,img:null,paths:['assets/home baked sprites/sine/SINE.gif']},
  {firstGid:8989,lastGid:8993,cols:5,img:null,paths:['assets/home baked sprites/square/omniblock.png','assets/home baked sprites/square/omniblock.gif']},
  {firstGid:8994,lastGid:8994,cols:1,img:null,paths:['assets/home baked sprites/material/brick.png']},
];
function _tmjImgSrc(path){ return encodeURI(path).replace(/#/g,'%23'); }
function _tmjLookupGid(gid){ if(!gid)return null; for(const ts of TMJ_TILESET_DEFS){if(gid>=ts.firstGid&&gid<=ts.lastGid&&ts.img)return{img:ts.img,tid:gid-ts.firstGid,cols:ts.cols};} if(_tmjImg&&gid<TMJ_TILESET_DEFS[1].firstGid)return{img:_tmjImg,tid:gid-1,cols:_tmjImgCols}; return null; }
...
function _canvasFloorFeetAt(cx,markerFeet,maxRows=10){
  const draw=_tmjDraw;
  if(!draw) return null;
  const data=draw.canvasData||draw.data;
...
function _isBasementSpawnRow(row){ return row!=null&&row>=TMJ_BASEMENT_ROW; }
...
function _isCollLayerName(name){ const l=String(name||'').toLowerCase().replace(/\\/g,'/');return l==='collision'||l==='walls/floors'||l==='walls'||l==='floors'||l==='wall/floor'||l==='floor/walls'; }
...
function _mkBwallFromBlockObj(o,tw,th,sc,destructData,canvasData,mw){
  ...
  const{tileGid,canvasGid,homeCol,homeRow}=_pickBwallTileGids({x:mapX,y:mapY,width:tw,height:th},tw,th,destructData,canvasData,mw);
  const gid=tileGid||TMJ_OMNIBLOCK_FIRST;
  return _mkBwall(x,y,w,h,{tileGid:gid,canvasGid,homeCol,homeRow,homeX:x,homeY:y,movable:false});
}
...
function _pickBwallTileGids(o,tw,th,destructData,canvasData,mw){
  ...
  let tileGid=0,canvasGid=0,homeCol=Math.floor(o.x/tw),homeRow=Math.floor(o.y/th);
  if(mw>0){
  ...
  return{tileGid,canvasGid,homeCol,homeRow};
}
...
function _bwallDestructSkipCells(){
  const skip=new Set();
  for(const bw of BWALLS){ if(bw.homeCol==null||bw.homeRow==null)continue; if(bw.hp>0&&!_bwallMovedFromHome(bw))skip.add(bw.homeRow+'_'+bw.homeCol); }
  return skip;
}
...
function _clearTiledState(){
  ...
  COLL_SEGS=[]; COLL_WALL_SEGS=[]; COLL_POLYS=[]; _useSegGround=false; BWALLS.length=0;
}
...
function applyTmjMap(data){
  ...
  COLL_SEGS=[]; COLL_WALL_SEGS=[]; COLL_POLYS=[];
  _useSegGround=false;
  ...
  for(const layer of layers){
    ...
    if(layer.type==='tilelayer'){
      ...
      if(_isCollLayerName(layer.name||'')){
      ...
      if(_isBlocksLayerName(layer.name||'')){
        for(const o of objects){
          const bw=_mkBwallFromBlockObj(o,tw,th,sc,destructData,canvasData,mw);
          if(bw) _mapBWalls.push(bw);
        }
      }
      if(_isDestructLayerName(layer.name||'')||_isManipLayerName(layer.name||'')){
        for(const o of objects){
          const{tileGid,canvasGid,homeCol,homeRow}=_pickBwallTileGids(o,tw,th,destructData,canvasData,mw);
          ...
          _mapBWalls.push(_mkBwall(x,y,w,h,{tileGid,canvasGid,homeCol,homeRow,homeX:x,homeY:y,movable:tileGid===BWALL_RED_GID}));
        }
      }
    }
  }
  // ── Collision geometry ────────────────────────────────────────
  // Polygon walk surfaces come from COLL_SEGS; skip thick TR floor plats (they
  // duplicate segments and embed the player when feet sit inside h=32 rects).
  const merged=_mergeCollRects(collRects);
  TR.push(...merged);
  for(const pts of collPolys){
    COLL_POLYS.push(pts);
    const segs=_buildTopWalkSegs(pts); COLL_SEGS.push(...segs);
    COLL_WALL_SEGS.push(..._buildWallSegs(pts));
  }
  ...
  _resolveAndApplySpawn(data,tw,th,sc,spawnTileLayer,mw);

  _mapApplied=true; _mapReady=true;
  if(p) _placePlayerAtSpawn();
  ...
  console.info('MV applyTmjMap:',WW+'x'+WH,TR.length,'rects,',COLL_SEGS.length,'segs,',
    ...
    '| spawn',_spawnX,_spawnY+FEET_OFF);
}

// ── Tiled draw helpers ────────────────────────────────────────
function _tmjDisplayGid(gid){
  if(gid>=TMJ_OMNIBLOCK_FIRST&&gid<TMJ_OMNIBLOCK_FIRST+TMJ_OMNIBLOCK_FRAMES){
    return TMJ_OMNIBLOCK_FIRST+(Math.floor(fr/8)%TMJ_OMNIBLOCK_FRAMES);
  }
  return gid;
}
function _drawTmjTileGid(gid,px,py,tw,th,sc){
  gid=_tmjDisplayGid(gid);
  const hit=_tmjLookupGid(gid);
  const dw=Math.max(1,Math.round(tw*sc)), dh=Math.max(1,Math.round(th*sc));
  if(hit){ const sx0=(hit.tid%hit.cols)*tw,sy0=(hit.tid/hit.cols|0)*th; if(_tmjChromaKey(gid)) blitImgChroma(hit.img,sx0,sy0,tw,th,px,py,dw,dh); else blitImg(hit.img,sx0,sy0,tw,th,px,py,dw,dh); }
...
function _drawTmjTileLayer(data,mw,mh,tw,th,sc,skipCells,skipGids,mode){
  ...
  for(let row=row0;row<=row1;row++){
    for(let col=col0;col<=col1;col++){
      const gid=_tmjGid(data[row*mw+col]); if(!gid)continue;
      if(skipGids&&skipGids.has(gid))continue;
      if(skipCells&&skipCells.has(row+'_'+col))continue;
      _drawTmjTileGid(gid,sx(col*tws),sy(row*ths),tw,th,sc);
    }
  }
}
function drawBwallHomeReveal(){ if(!_tmjDraw||(!_tmjImg&&!TMJ_TILESET_DEFS.some(t=>t.img)))return; const{tw,th,sc}=_tmjDraw,tws=tw*sc,ths=th*sc; for(const bw of BWALLS){if(bw.homeCol==null||!bw.canvasGid)continue;if(bw.hp>0&&!_bwallMovedFromHome(bw))continue;_drawTmjTileGid(bw.canvasGid,sx(bw.homeCol*tws),sy(bw.homeRow*ths),tw,th,sc);} }
function drawTmjMap(){
  if(!_tmjDraw) return;
...
(function _loadTmjTilesets(){
  TMJ_TILESET_DEFS.forEach(ts=>{
    let i=0;
    const next=()=>{
      if(i>=ts.paths.length){console.warn('MV: missing tileset gid',ts.firstGid,'tried',ts.paths.join(', '));return;}
      const path=ts.paths[i++];
      const img=new Image();
      if(location.protocol!=='file:') img.crossOrigin='anonymous';
      img.onload=()=>{
        const autoCols=Math.max(1,Math.floor(img.width/8));
        if(ts.firstGid===1) ts.cols=autoCols;
        ts.img=MV2_VISUAL?img:smsProcessPixelArt(img);
        if(ts.firstGid===1){_tmjImg=ts.img;_tmjImgCols=ts.cols;}
        console.info('MV tileset loaded:',path,'gid',ts.firstGid+'-'+ts.lastGid,'cols',ts.cols);
...
function initWorld(){
  ...
  const boot=()=>{
    ...
    ENEMS=[]; CRATES=[]; _restoreMapBWalls(); _resetItemProgress(); _populateKnowlFromMap(); _initLaituFromMap();
    ...
    _zoneIdx=0; _zoneCardT=220; GOALPL={x:-999,y:0,w:1,h:1};
...
function update(){
  ...
  if(p.inv>0)p.inv=0;if(p.landF>0)p.landF--;if(p.flashF>0)p.flashF--;
  ...
  for(const e of ENEMS){
    if(e.mind){
      ...
      if(_enemyBodyPresent(e)) _resolvePlayerEnemySeparation(p,e,_battleTestMode?4:3);
      continue;
    }
    ...
    if(e.alive&&_shutdownTimer<=0){const _pCoreE=playerCoreHB(p);const _eh=_enemyItemHB(e);if(ov(_pCoreE.x,_pCoreE.y,_pCoreE.w,_pCoreE.h,_eh.x,_eh.y,_eh.w,_eh.h)){applyPlayerHitFromEnemy(e);}}
    _tickRivalMinionSpawn(e);
  }
  _resolveMindEnemiesSeparation();
  if(_battleTestMode){for(const e of ENEMS){if(e.mind&&_enemyBodyPresent(e)) _resolvePlayerEnemySeparation(p,e,2);}}
  ...
  if(p.hook.st!=='on'&&p.wallGrip<=0){
    p.crouchInput=!!(p.og&&isDn());const airDuck=!p.og&&isDn();const headroom=measureHeadroom(p);const tunnelNeed=headroom<STAND_H-8?Math.min(1,(STAND_H-8-headroom)/(STAND_H-DUCK_H)):0;const target=Math.max((p.crouchInput||airDuck)?1:0,tunnelNeed);p.crouchAmt+=(target-p.crouchAmt)*(target>p.crouchAmt?0.28:0.16);if(p.crouchAmt<0.02)p.crouchAmt=0;if(p.crouchAmt>0.98)p.crouchAmt=1;
    if(p.og&&isDn()){p.duckCharge=Math.min(90,(p.duckCharge||0)+1);if(p.duckCharge>=60&&!p.duckBoostReady){p.duckBoostReady=true;sfx('chargeReady');}}
    else if(p.og){p.duckCharge=Math.max(0,(p.duckCharge||0)-2);if(p.duckCharge<40)p.duckBoostReady=false;}
    ...
    if(isJumpJ()&&p.og){
      ...
      p.og=false;p._peakVy=0;const takeVy=Math.abs(p.vy);p._jumpAmp=Math.min(1,Math.max(0.25,takeVy/7.2));p.jumpTiltF=Math.round(14+p._jumpAmp*16);const jhx=Math.abs(p.vx)>0.25?p.vx:(p.fc?1:-1)*0.55;const jhy=p.vy<-0.2?-1:(p.vy>0.5?0.35:-0.75);const jhl=Math.hypot(jhx,jhy)||1;p._jumpHatDX=jhx/jhl;p._jumpHatDY=jhy/jhl;
    }
    ...
    p.vy=Math.min(p.vy+(isJump()&&p.jf>0&&p.vy<0?GRAV*0.38:GRAV),14);
    if(!_playerOnGround(p)&&p.hook.st!=='on'&&p.wallGrip<=0) p._peakVy=Math.max(p._peakVy||0,p.vy);
    _movePlayerWithColl(p,0,p.vy);
    if(Math.abs(p.vx)>0.01){const _ox=p.x;_movePlayerWithColl(p,p.vx,0);if(_playerOnGround(p))p.vx=p.x-_ox;}
    _zeroVxIntoWall(p);
  ...
  updateBWalls(); _resolvePlayerOutOfBwalls(p); if(p2)_resolvePlayerOutOfBwalls(p2);
  ...
  }else if(!win&&goalOpen&&(_zoneIdx>0||_stageDesignerMode)&&(p1Goal||p2Goal)){
...
function loop(){
  ...
  if(!_editorActive){ try{update();}catch(err){console.error('update error',err);} _syncGameCamera(); }
